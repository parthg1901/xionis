...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::CreateProposal { desc, title } => {
            execute::try_create_proposal(deps, _env, info, desc, title)
        }
...
pub mod execute {
    ...
    pub fn try_vote(
        ...
        let mut proposal = match PROPOSALS.load(deps.storage, id.into()) {
...
pub mod query {
    ...
    use crate::{
        msg::{Members, Proposals},
        state::{MemeberType, Proposal, MEMBERS, PROPOSALS},
    };
    ...
    pub fn query_proposals(deps: Deps) -> Result<QueryResponse, StdError> {
        let keys: Result<Vec<_>, _> = PROPOSALS
            .keys(deps.storage, None, None, Order::Ascending)
            .collect();

        let proposals: Result<Vec<Proposal>, _> = keys?
            .into_iter()
            .map(|key| {
                let id = key;
